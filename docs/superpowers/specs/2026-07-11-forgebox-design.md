# ForgeBox — Design Spec

**Date:** 2026-07-11
**Status:** Approved by Ahmad (design conversation, 2026-07-11)
**Repo:** `ForceAI-KW/forgebox` · Apache-2.0 · `~/Desktop/projects/forgebox`
...
3. **Networking niceties** — VPN-compatible NAT, `<name>.fbox.local` DNS for machines/containers, automatic port forwarding to localhost.
...
Working name **ForgeBox**: daemon `fboxd`, CLI `fbox`, app `ForgeBox.app`, state dir `~/.fbox/`, docker context `forgebox`, DNS zone `*.fbox.local`. Rename is a find/replace away if Ahmad picks another name.

## Architecture

```
┌─ ForgeBox.app (SwiftUI menu bar, thin client) ─┐   ┌─ fbox (Go CLI) ─┐
└──────────────┬─────────────────────────────────┘   └───────┬─────────┘
               │        JSON API over ~/.fbox/run/api.sock   │
        ┌──────▼──────────────────────────────────────────────▼──────┐
        │  fboxd — Go daemon (launchd LaunchAgent, KeepAlive)        │
        │  • Code-Hex/vz → Virtualization.framework                  │
        │  • gvisor-tap-vsock embedded in-process (net + DNS + fwd)  │
        │  • state: ~/.fbox/{machines,images,run,log}                │
        └──┬──────────────────────────┬──────────────────────────────┘
           │                          │
   ┌───────▼────────┐        ┌────────▼─────────┐
   │ docker VM       │        │ machines          │
...
**`fboxd` (Go daemon)**
- Runs as a launchd LaunchAgent (`com.forceai.fboxd`), starts at login, `KeepAlive` restart on crash.
- Owns all VMs via `Code-Hex/vz` (the Virtualization.framework Go bindings Lima uses).
- JSON-over-unix-socket API at `~/.fbox/run/api.sock` (single consumer surface for CLI + GUI).
- Machine registry + config in `~/.fbox/machines/<name>/config.json`; raw disk images alongside.
...
- `fbox shell <machine>` → SSH over the gvisor virtual network (key managed by fboxd).
...
- dockerd's socket exposed to the host at `~/.fbox/run/docker.sock` (forwarded over vsock/virtual net).
- `fbox docker install` registers docker context `forgebox` and sets it current.
...
- Built-in DNS server: `<machine>.fbox.local` for machines; `<container>.fbox.local` for docker containers (fboxd watches docker events to register names). Host resolver hookup via `/etc/resolver/fbox.local`.
- Port forwarding: fboxd watches docker for published ports and forwards them to localhost automatically; machines get explicit `fbox forward` plus sensible defaults (SSH).

**CLI `fbox`**
- `fbox create|start|stop|rm|list <machine>`, `fbox shell <machine>`, `fbox docker install|start|stop`, `fbox status [--json]`, `fbox logs`, `fbox forward`.
- `fbox status --json` is the probe surface for the self-healing OS watchdog.
...
- CLI/GUI → JSON API on `api.sock` → fboxd → vz VM operations.
- `docker` CLI → `~/.fbox/run/docker.sock` → stream forwarded into docker VM → dockerd.
- Guest traffic → gvisor-tap-vsock userspace stack → host sockets (NAT); inbound via explicit forwards.

## Error handling & reliability

- **Daemon crash = VMs down** (VZ VMs live in the daemon process; OrbStack has the same shape). Mitigations: minimal daemon surface, launchd KeepAlive, autostart machines re-boot in seconds, watchdog probes `fbox status --json`.
- Graceful shutdown: SIGTERM → ACPI shutdown request to guests → 30s → force stop.
- Disk safety: machine disks are plain raw files under `~/.fbox`; they ride the existing nightly Mac backup discipline. `fbox rm` requires the machine be stopped; no cascade deletes.
- Logs: per-machine console logs + daemon log under `~/.fbox/log/`.
...
- **E2E smoke**: create machine → `docker run hello-world` → published-port reachable on localhost → `dig <machine>.fbox.local` resolves. Runs on this Mac (GitHub-hosted macOS runners can't nest VZ); later on the Mac Studio self-hosted runner.
- **UAT**: `docs/uat/cutover.md` — the fwb-ci cutover checklist (below) is the acceptance test.

## Milestones

1. **M1 — Core VM lifecycle**: fboxd + `fbox`; Ubuntu machine boots via cloud-init; `fbox shell` works; VirtioFS home mount.
2. **M2 — Networking**: gvisor-tap-vsock NAT; `.fbox.local` DNS + host resolver; port forwarding.
3. **M3 — Docker**: docker VM, socket forward, `forgebox` context; build/run/compose parity; docker-event-driven DNS + auto port forwarding.
4. **M4 — Autostart + fwb-ci cutover**: LaunchAgent, autostart flag, watchdog probe. Cutover: fresh `fwb-ci2` machine in ForgeBox → install + register runners **in parallel** with OrbStack's → verify green CI runs → deregister old runners → retire OrbStack machine → uninstall OrbStack.
...
- fwb-ci runners online under ForgeBox, OrbStack uninstalled.
- `docker` context `forgebox` is the daily driver (build/run/compose verified).
